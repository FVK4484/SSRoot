...
DWORD WINAPI Incrementa(LPVOID NoSeUsa)
{
	...
	for (i = 0; i<NRO_INCREMENTOS; i++)
	{
		...
		CheckError(0 != WaitForSingleObject(hMutex, INFINITE), "Error en el bloqueo", 1);
		Contador++;
		// Indica que el recurso está libre
		CheckError(FALSE==ReleaseMutex(hMutex), "No se pudo desbloquear el objeto", 1);
	}
...
int main(int argc, char *argv[])
{
	...
	CheckError(hMutex == NULL, "No se pudo crear el objeto de sincronización", 1);
