...
DWORD WINAPI Incrementa(LPVOID NoSeUsa)
{
	...
	for (i = 0; i<NRO_INCREMENTOS; i++)
	{
		...
		CheckError(0 != WaitForSingleObject(hSemaforo, INFINITE), "Error en el bloqueo", 1);
		Contador++;
		// Indica que el recurso está libre
		CheckError(FALSE == ReleaseSemaphore(hSemaforo, 1, NULL), "No se pudo desbloquear el objeto", 1);
	}
...
int main(int argc, char *argv[])
{
	...
	// Inicializa objeto
	hSemaforo = CreateSemaphore(NULL, 1, 1, NULL); // Reset automatico, inicialmente abierto
	CheckError(hSemaforo == NULL, "No se pudo crear el objeto de sincronización", 1);
	...
	printf("\n La diferencia en los valores es %d  (protegido con semáforo)", NRO_HILOS*NRO_INCREMENTOS - Contador);
