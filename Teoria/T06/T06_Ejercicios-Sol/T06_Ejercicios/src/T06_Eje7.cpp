...
DWORD WINAPI Incrementa(LPVOID nroDeHilo)
{
	int i;
	printf("\n>> Entrando en hilo %d\n", (int)nroDeHilo);
...
int main(int argc, char *argv[])
{
	...
	{
		hThreads[i] = CreateThread(NULL, 0, Incrementa, (LPVOID)i, 0, NULL); // ¿Por qué no se le pasa &i en lugar de i?
	...
	printf("\n La diferencia en los valores es %d  (protegido con sección crítica)",	NRO_HILOS*NRO_INCREMENTOS - Contador);
