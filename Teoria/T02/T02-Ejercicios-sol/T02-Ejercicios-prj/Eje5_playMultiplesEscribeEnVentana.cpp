...
int main(int argc, char *argv[]) {
	...
	for (int i = 1; i < argc ; i++) {
		_snprintf(nombreVentana, sizeof(nombreVentana), "Reproduciendo: %s", argv[i]);
...
static void play(const char *file_name, const char * nombre_ventana)
{
	...
	else
	{
		...
		while (newframe && (key != 27)) {
			...
			// Añade texto al frame
			//cvPutText(CvArr* img, const char* text, CvPoint org,
			//	const CvFont* font, CvScalar color);
			// Ver https://docs.opencv.org/3.0-last-rst/modules/imgproc/doc/drawing_functions.html?highlight=cvputtext#puttext
			// La fuente debe inicializarse usando // Se inicializa usando https://docs.opencv.org/3.0-last-rst/modules/imgproc/doc/drawing_functions.html?highlight=cvfont
